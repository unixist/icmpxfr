...
void fill_icmp_packet(struct icmp *pkt, struct icmp *last_pkt, unsigned char *payload, size_t payload_len, size_t icmp_len){
   pkt->icmp_type = ICMP_ECHO;
   pkt->icmp_code = 0;
   pkt->icmp_id = last_pkt->icmp_id;
...
int main (int argc, char **argv){
   ...
   while ((opt = getopt(argc, argv, "hd:cf:")) != -1) {
      switch (opt) {
      case 'f':
         filename = optarg;
         break;
   ...
   fill_ip_packet(ip_hdr_out, sizeof(sha1_hash), dst_ip);
   ip_len = ntohs(ip_hdr_out->ip_len);
   icmp_len = ip_len - sizeof(struct iphdr);
